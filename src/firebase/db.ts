...
function getDataOnce(path: string) {
  const { uid } = getUser()
  return get(ref(db, `users/${uid}/${path}`))
...
export async function createNewFruit(newFruitInfo: Omit<Fruit, 'id'>): Promise<Fruit> {
...
export async function updateFruit(id: string, newRating: number): Promise<Fruit> {
  const currentFruits = await getDataOnce('fruits') as Fruit[]
  ...
  return setData('fruits', newFruits)
    .then(() => getOneFruit(id) as unknown as Fruit)
}
